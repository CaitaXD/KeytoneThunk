...
public class MidiExportFileMusicPlayerStrategy : IMusicPlayerStrategy
{
    ...
    {
        int quarterNoteTicks = Ticks(duration)/4;
        int noteNumber = MidiConverter.Note(note, octave);
        var startNodeEvent = new NoteOnEvent(_timeStamp, Channel, noteNumber, Volume, quarterNoteTicks);
        var stopNodeEvent = new NoteOnEvent(_timeStamp + quarterNoteTicks, Channel, noteNumber, 0, 0);
        _midiEvents.AddEvent(startNodeEvent, TrackNumber);
        _midiEvents.AddEvent(stopNodeEvent, TrackNumber);
        _timeStamp += quarterNoteTicks;
